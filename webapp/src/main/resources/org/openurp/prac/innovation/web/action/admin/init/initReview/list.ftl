...
[@b.grid items=reviews var="review"]
  [@b.gridbar]
    ...
    [/#list]
    bar.addItem("${b.text("action.export")}",action.exportData("project.code:编号,project.title:项目名称,project.manager.std.user.name:负责人,project.department.name:部门,score:评审分数",null,'fileName=立项评审信息'));
  [/@]
  [@b.row]
    [@b.boxcol /]
    [@b.col width="11%" property="project.code" title="编号"/]
    [@b.col width="32%" property="project.title" title="名称"]<span style="font-size:0.8em">${review.project.title}</span>[/@]
