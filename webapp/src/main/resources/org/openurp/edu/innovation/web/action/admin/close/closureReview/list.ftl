...
[@b.grid items=reviews var="review"]
  [@b.gridbar]
    ...
    [/#list]
    bar.addItem("${b.text("action.export")}",action.exportData("project.code:编号,project.title:项目名称,project.manager.std.user.name:负责人,project.department.name:部门,score:评审分数,",null,'fileName=结项评审信息'));
  [/@]
  [@b.row]
    [@b.boxcol /]
