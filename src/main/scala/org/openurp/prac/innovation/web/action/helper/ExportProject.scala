...
class ExportProject(project: Project) {
  ...
  def getManagerName: String = {
    project.manager.get.std.name
  }
  def getManagerCode: String = {
    project.manager.get.std.code
  }
  ...
  def getMemberNames: String = {
    ...
    val names = project.members map { x =>
      i += 1;
      x.std.name + " " + i + "/" + x.std.code
    }
    names.mkString(",")
  }

  def getInstructorNames: String = {
    project.instructors.map(_.name).mkString(",")
  }

  def getInstructorTitles: String = {
    project.instructors.map(_.staff.title.map(_.name).getOrElse("")).mkString(",")
  }
  ...
  def getSummary: String = {
    project.intro.map(_.summaries).getOrElse("--")
  }
